...
The project provides governance templates, Codex and Claude Code plugin surfaces, deterministic helper scripts, marketplace examples, user-level Git install/update support for local marketplaces, Harness v6.2.6 automatic natural-prompt Project Governor entrypoints, Fast Path v2 task routing with `tiny_patch` and `artifact_policy`, GPT-5.5-aware runtime planning, compact context indexing with `DOCS_MANIFEST.json`, architecture-aware roles, and section-level retrieval, governed memory search, session-learning ledgers for failed commands and stale memory, session state, evidence manifests, engineering standards checks for source size, complexity, mock leakage, tests, reuse, and command execution policy, quality-gated acceleration workflows, smart route guards, subagent selection with host-runtime authorization awareness, plugin upgrade migration tools with AGENTS.md/CLAUDE.md rule-template drift detection, opt-in design-system governance assets, DESIGN.md-gated UI workflows with full-service or basic mode, diagnostics-baseline cleanup, skill catalog consolidation, and scheduled-memory examples so future Codex or Claude Code sessions can preserve project rules, conventions, decisions, risks, design constraints, and durable memory while still moving quickly.
