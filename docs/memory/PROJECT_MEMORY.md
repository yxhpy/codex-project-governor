...
| Current phase | v6.0.5 Project Governor Harness with session-learning ledgers for failed commands and stale memory, proactive memory-search startup policy, user-level Git install/update for local marketplaces, single-source task routing, GPT-5.5 runtime planning, context-index v2, governed memory search, AGENTS.md rule-template drift migration, session lifecycle state, evidence manifests, diff-derived route guard facts, DESIGN.md governance, DESIGN.md-gated UI workflows, clean reinstall management, plugin upgrade migration, routing, quality gates, and self-tests | `.codex-plugin/plugin.json`, `README.md`, `README.zh-CN.md`, `skills/memory-compact/scripts/record_session_learning.py`, `templates/.project-governor/state/COMMAND_LEARNINGS.json`, `tests/test_session_learning.py` | confirmed |
...
- Date: 2026-04-29
- Reviewer: Codex v6.0.5 patch release workflow
